% Computes correlation on each window to see evolution in time
...
for s_which=1:length(m_upstates)
    ...
    v_vmUpstate= m_upstates{s_which,1};
    v_EEG_upstates = m_upstates{s_which,2};
    b_valid = validity_test(v_vmUpstate);
    ...
    if b_valid
        ...
        filtre_upstate(v_vmUpstate,v_EEG_upstates,info(1).header.Sampling);
